...
INSERT INTO films (director_id, title, year, rating, language)
VALUES (1, 'Titanic', 1997, 7.8, 'English'),
       (2, 'Saving Private Ryan', 1998, 8.6, 'English'),
       (3, 'Inception', 2010, 8.8, 'English'),
       (3, 'Interstellar', 2014, 8.6, 'English');

INSERT INTO technical_specs (film_id, runtime, camera, sound)
VALUES (1, '03:14:00', 'Panavision Cameras', 'Dolby Digital'),
       (2, '02:49:00', 'Arriflex Cameras', 'DTS'),
       (3, '02:28:00', 'Panavision Cameras', 'Dolby Digital'),
       (4, '02:49:00', 'IMAX Cameras', 'Dolby Atmos');

INSERT INTO roles (film_id, actor_id, character)
...
INSERT INTO competitions (name, year)
VALUES ('Academy Awards', 1998),
       ('Golden Globes', 1998),
       ('Academy Awards', 2011),
       ('Cannes Film Festival', 2014);

INSERT INTO nominations (competition_id, film_id, rank)
VALUES (1, 1, 1), -- Titanic Oscar
...
BEGIN
  IF
NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'web_anon') THEN
...
BEGIN
  IF
NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'authenticator') THEN
